#!/bin/bash
# LinkedIn 登录脚本 - 使用 agent-browser (Playwright)
# 架构：连接到常驻 Chrome (CDP 9222)，登录态自动保留在 PVC
# 用法: ./login-linkedin.sh
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "请在浏览器中打开上述链接，输入邮箱和密码登录。"
echo "如需 2FA 验证，请完成验证。"
...
if echo "$CONTENT" | grep -qi "feed\|linkedin"; then
    echo "✅ 登录成功！"
  ...
  res.on('end', () => {
    ...
    ws.on('open', () => {
      ws.send(JSON.stringify({id: 1, method: 'Runtime.evaluate', params: {
        expression: 'document.querySelector(\".global-nav__me-link\")?.innerText || document.querySelector(\"[data-control-name=identity_welcome_message]\")?.innerText || \"\"'
      }}));
    });
    ws.on('message', (msg) => {
      const result = JSON.parse(msg);
